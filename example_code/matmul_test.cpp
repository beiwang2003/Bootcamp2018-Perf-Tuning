/******************************************************************************************************
  ...
  Compile:
     icpc -g -mkl -O3 -xhost matmul_test.cpp -o mm.out
...
int main(int argc, char *argv[])
{
  ...
  timeval t1, t2, tl1, tl2, tl3, tl4; //timer
  gettimeofday(&t1, NULL); // call timer
  ...
  gettimeofday(&t2, NULL); // call timer
  timersub(&t2, &t1, &tl1); // get elapsed time
  cout <<"loop for mm1 finished in "
       << tl1.tv_sec + tl1.tv_usec/1000000.0
       << " seconds" << endl;
  ...
  gettimeofday(&t1, NULL); // call timer
  timersub(&t1, &t2, &tl2); // get elapsed time
  cout <<"loop for mm2 finished in "
       << tl2.tv_sec + tl2.tv_usec/1000000.0
       << " seconds" << endl;
  ...
  gettimeofday(&t2, NULL); // call timer
  timersub(&t2, &t1, &tl3); // get elapsed time
  cout <<"loop for mm3 finished in "
       << tl3.tv_sec + tl3.tv_usec/1000000.0
       << " seconds" << endl;
  ...
  gettimeofday(&t1, NULL); // call timer
  timersub(&t1, &t2, &tl4); // get elapsed time
  cout <<"loop for mm4 finished in "
       << tl4.tv_sec + tl4.tv_usec/1000000.0
       << " seconds" << endl;
